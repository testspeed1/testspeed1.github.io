...
async function start() {
    ...
    if (testurl.length < 10) {
        alert("链接不合法")
        $('#do').attr('onclick', 'botton_clicked()');
        return;
    }
    ...
    //加载图标
    $("#kzimg").attr('class', 'fa fa-spinner fa-pulse fa-2x');
    // document.getElementById('kzimg').src = 'jz.png';
    // document.getElementById('kzimg').style.marginRight = "6px";

    // var image = document.getElementById("kzimg");
    // var angle = 0;
    ...
    // document.getElementById('kzimg').marginLeft = "5px";
    // document.getElementById('do').innerText = '正在检验';
    ...
    try {
        ...
        $('#do').attr('onclick', 'botton_clicked()');
        //开始图标
        $("#kzimg").attr('class', 'fa fa fa-play fa-2x');

        // document.getElementById('do').innerText = '开始';
        // var img = document.getElementsByClassName('kzimg')[0];
        // img.src = 'bf.png';


        // document.getElementById('kzimg').src = 'bf.png';
        // document.getElementById('kzimg').style.marginRight = "0px";
        // var image = document.getElementById("kzimg");
    ...
    // document.getElementById('kzimg').src = 'zt.png';
    // document.getElementById('kzimg').style.marginRight = "6px";
    // clearInterval(interval);
    // var image = document.getElementById("kzimg");
    // image.style.transform = "rotate(0deg)";
    ...
    //停止图标
    $("#kzimg").attr('class', 'fa fa-pause fa-2x');
...
function ipcn() {
    if (visibl) {
        fetch('https://forge.speedtest.cn/api/location/info', { referrerPolicy: 'no-referrer' })
            ...
            .then(data => {
                var tag = document.getElementById("ipcn")
                tag.innerText = data['province'] + ' ' + data['city'] + ' ' + data['distinct'] + ' ' + data['isp']
                if (data['ip'] !== cnip) {
                    tag.style.color = ''
                    ckip(data['ip'], tag)
                }
...
function ipcn2() {
    if (visibl) {
        fetch('https://forge.speedtest.cn/api/location/info', { referrerPolicy: 'no-referrer' })
            .then(response => response.json())
            .then(data => {
                var tag = document.getElementById("ipcn2")
                tag.innerText = data['ip']
                if (data['ip'] !== cnip) {
                    tag.style.color = ''
                    ckip(data['ip'], tag)
                }
...
//     }
//     document.getElementById('link').value = this.options[this.selectedIndex].value; setCookie('select', this.selectedIndex, 365);


// }

// 要操作的元素
const rangeValue = document.querySelector('#thread1');
